...
std::map<const reco::Candidate *, int> genCandidatesBarcode;
for (auto p = genCandidatesCollection->begin(); p != genCandidatesCollection->end(); ++p) {
  genCandidatesBarcode[&(*p)] = barcode;
  barcode++;
}
...
///particle info
GEN_barcode.push_back(genCandidatesBarcode[&(*p)]);
...
if(Nmothers>0) {
   ...
   Temp_GEN_Mom_barcode.push_back(genCandidatesBarcode[p->mother(i)]);
   }
...
if(Ndaughters>0) {
   ...
   Temp_GEN_Dau_barcode.push_back(genCandidatesBarcode[p->daughter(i)]);
   }
    ...
    if(Nmothers>0){
        ...
        if (!(Mom->pdgId()==443 && Mom->status()==2)) continue;  // Upsilon =553
         ...
         if(verbose) cout<<"found a GEN  MUON ID : "<< p->pdgId() <<"GEN MUON status :"<<  p->status() << "GEN MUON mass: "<< p->mass() << " GEN MUON pt: " << p->pt() <<" GEN MUON eta: "<< p->eta() <<endl;
           ...
           GENmu_barcode.push_back(genCandidatesBarcode[&(*p)]);
           GENmu_Mom_barcode.push_back(genCandidatesBarcode[Mom]);
           GENmu_MomMom_barcode.push_back(genCandidatesBarcode[GrandMom]); 
             } // end Gen muons
            ...
            GENups_NDaughters.push_back(ndau); 
                for (int d=0; d<ndau; d++) {
                    ...
                    { //if daughter of ups is muon
                        ...
                        Temp_GENups_Daughter_barcode.push_back(genCandidatesBarcode[ups_dau]);
                    }
            ...
            GENups_Mom_barcode.push_back(genCandidatesBarcode[Mom]);                
            ...
            GENups_barcode.push_back(genCandidatesBarcode[&(*p)]);
                } // Upsilon loop
            } //if Gndmother>0
        }//if mother>0
