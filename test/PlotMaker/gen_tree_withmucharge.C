...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   Long64_t nbytes = 0, nb = 0;
    ...
    TFile *myfile = new TFile("8Nov2020_DPSJJ_with_genfilter_2018_NO_etptcut_muonid_J1-J2PT.root","RECREATE");
...
//Muons ID
        FourMu_tree->Branch("mu1_id" ,&mu1_id ,"mu1_id/D");
        FourMu_tree->Branch("mu2_id" ,&mu2_id ,"mu2_id/D");
        FourMu_tree->Branch("mu3_id" ,&mu3_id ,"mu3_id/D");
        FourMu_tree->Branch("mu4_id" ,&mu4_id ,"mu4_id/D");
...
/// Upsilon dongusu
     ...
     {  
        ...
        for (size_t k = j+1; k < GENups_ID->size(); k++)
        {
          ...
          //question: 
          //The gen_tree did not have charge or pdg code info for muon in before version. Did you considering the charge when do the single muon mix?
          // we can make charge selection of  muons from muon pdg_ID which in previous step (ntplmaker)
          //muons charge condition
              if( ( (*GENups_Daughter_muid)[j][0] == (*GENups_Daughter_muid)[j][1] ) || ((*GENups_Daughter_muid)[k][0] == (*GENups_Daughter_muid)[k][1])) continue;
          // another method for charge condition:
          // if ( ( (*GENups_Daughter_muid)[j][0] * (*GENups_Daughter_muid)[j][1] ) < 0 || ((*GENups_Daughter_muid)[k][0] *  (*GENups_Daughter_muid)[k][1])<0 ) continue;
          //
          ...
          //std::cout<<" pt : "<< (*GENups_Daughter_mupt)[j][0] <<" pt : "<< (*GENups_Daughter_mupt)[j][1]<<" pt : "<< (*GENups_Daughter_mupt)[k][0]<<" pt : "<< (*GENups_Daughter_mupt)[k][1]<<std::endl;;
           ...
           mu34=J34;
            ...
            //std::cout<<"Jpsimass_1 :"<<J12.M()<<std::endl;
            //std::cout<<"Jpsimass_2 :"<<J34.M()<<std::endl;
            ...
            //std::cout<<"FourMuon Mass :"<<Fourmu.M()<<std::endl;
            //std::cout<<"Jpsimass_1_1 :"<<J12.M()<<std::endl;
            //std::cout<<"Jpsimass_2_2 :"<<J34.M()<<std::endl;
            ...
            /*upsilon*/
              ...
              Y34_upsilon    =  J34.Rapidity();
              
              /*muons*/
              
              mu1_id     = (*GENups_Daughter_muid)[j][0] ;
              mu2_id     = (*GENups_Daughter_muid)[j][1] ;
              mu3_id     = (*GENups_Daughter_muid)[k][0] ;
              mu4_id     = (*GENups_Daughter_muid)[k][1] ;
