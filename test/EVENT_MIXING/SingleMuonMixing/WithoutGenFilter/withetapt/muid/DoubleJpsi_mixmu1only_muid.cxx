...
void DoubleJpsi_mixmu1only_muid(){

    TFile *f = new TFile("../DPSJJ_without_genfilter_2018_with_etptcut_muonid.root");  
    TTree *fTree = (TTree*) f->Get("FourMu_tree");
 
    FMu_muid *ReadTree = new FMu_muid(fTree);
	...
	std::vector<double> mu1_id_vector;
	std::vector<double> mu2_id_vector;
	std::vector<double> mu3_id_vector;
	std::vector<double> mu4_id_vector;
   ...
   cout<<"\r Read Event:"<< i <<"/"<<Ntotal<<flush;
		...
		double mu1_idx= ReadTree->mu1_id;
		double mu2_idx= ReadTree->mu2_id;
		double mu3_idx= ReadTree->mu3_id;
		double mu4_idx= ReadTree->mu4_id;
		J12=mu1+mu2;
		J34=mu3+mu4;
		...
		mu1_id_vector.push_back(mu1_idx);
		mu2_id_vector.push_back(mu2_idx);
		mu3_id_vector.push_back(mu3_idx);
		mu4_id_vector.push_back(mu4_idx);
...
int num =mu34_p4_vector.size();
	for (unsigned j=0; j<mu2_p4_vector.size();j++){ 
		for (unsigned k=0; k<mu1_p4_vector.size();k++){
			if (j==k) continue;//ayni eventte olmasinlar
			if (mu1_id_vector[k]==mu2_id_vector[j]) continue;
			//fiziksel yontem, old deltaR method. 
			if ( fabs(mu1_p4_vector[k].Vect().DeltaR( mu1_p4_vector[j].Vect())) > 0.1) continue;
