...
int num =mu34_p4_vector.size();
	for (unsigned j=0; j<mu2_p4_vector.size();j++){ 
		for (unsigned k=0; k<mu1_p4_vector.size();k++){
			...
			if ( fabs(mu1_p4_vector[k].Vect().DeltaR( mu1_p4_vector[j].Vect())) > 0.3) continue;
...
//mixing yontem mu12 ile mu34 mix et. 
	...
	cmix->SaveAs("plots_witgenfilt_NO_etaptcuts_SingleMuonMix_DR03/cmix.png");
        c4mu->cd();
	...
	hmix->SetLineColor(kRed);
	hmix->Draw("e1same");
        c4mu->SaveAs("plots_witgenfilt_NO_etaptcuts_SingleMuonMix_DR03/c4mu.png");
	c2j->cd();
	hMDoubleJpsi->Draw("e1");
        c2j->SaveAs("plots_witgenfilt_NO_etaptcuts_SingleMuonMix_DR03/c2j.png");
