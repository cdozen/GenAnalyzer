...
void DoubleJpsi_mixmu1only_015(){

    TFile *f = new TFile("../../../7Nov2020_DPSJJ_WITH_genfilter_2018_NO_etptcut.root");  
    //TFile *f = new TFile("/Users/candandozen/Desktop/Gen_level_eventmixing/TruthNtupMaker/Event_mixing_oldDRmethod/WithGenfilter/DPSJJ_withgenfiler_2018_w_etptcut.root");  
...
int num =mu34_p4_vector.size();
	for (unsigned j=0; j<mu2_p4_vector.size();j++){ 
		for (unsigned k=0; k<mu1_p4_vector.size();k++){
			...
			if ( fabs(mu1_p4_vector[k].Vect().DeltaR( mu1_p4_vector[j].Vect())) > 0.15) continue;
...
//mixing yontem mu12 ile mu34 mix et. 
	...
	cmix->SaveAs("plots_witgenfilt_NO_etaptcuts_SingleMuonMix_DR015/cmix.png");
        c4mu->cd();
	...
	hmix->SetLineColor(kRed);
	hmix->Draw("e1same");
        c4mu->SaveAs("plots_witgenfilt_NO_etaptcuts_SingleMuonMix_DR015/c4mu.png");
	c2j->cd();
	hMDoubleJpsi->Draw("e1");
        c2j->SaveAs("plots_witgenfilt_NO_etaptcuts_SingleMuonMix_DR015/c2j.png");
